...
sentences = crfChain_initSentences(y); % first column start, second is corresponding end indices
nSentences = size(sentences,1);

wv = [w(:);v_start(:);v_end(:);v(:)]; %combine four weights for c implementations

%% Set up training/testing indices
trainNdx = 1:floor(nSentences/2); %half for traing, half for test.
...
s = 1;
if useMex
    [nodePot,edgePot] = crfChain_makePotentials(X,wv,nFeatures,featureStart,sentences,s,nStates);
